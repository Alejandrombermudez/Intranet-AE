...
| **El módulo de Siembra lee columnas que ya no existen** | Las 4 páginas de `/intranet/siembra` piden `nombre_propietario`, `municipio`, `vereda` y `nombre_finca` de `siembra.familias`, que **`migration_campo_core.sql` eliminó el 2026-07-07** (esa identidad vive ahora en `core.predios`/`core.aliados`). La consulta devuelve `42703` y la pantalla decía "0 familias" con 2 familias vivas en la base. Es el mismo bug que se arregló en GeoAE el 2026-07-16 — aquí nadie lo vio porque **al módulo no se llegaba por ningún enlace**. La página de editar además *escribe* en esas columnas | `app/intranet/siembra/` (listado, eliminadas, detalle, editar). El listado ya **avisa en pantalla** en vez de mentir; falta resolver la identidad por JOIN a `core`, como en `GeoAE/lib/queries.ts` | 🆕 (hallado 2026-09-20) |
| ~~Siembra vivía bajo `/intranet/ras/` y nadie podía llegar~~ | El módulo lee `siembra.*`, no `ras.*`, pero estaba en `/intranet/ras/siembra`, protegido por `department === 'RAS'` y **sin un solo enlace que llevara hasta él**. Quien tuviera el departamento `Siembra` veía "módulo en construcción" aunque el módulo existía | Movido a `/intranet/siembra`, con entrada en `RUTA_MODULO` del hub, permisos en `lib/departamentos.ts` (`Siembra` + `RAS`) y un redirect en la ruta vieja para los marcadores | ✅ (2026-09-20) |
