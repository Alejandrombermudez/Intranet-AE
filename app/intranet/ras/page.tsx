...
import {
  ...
  CalendarDays, ChevronRight, Trash2, AlertTriangle,
} from 'lucide-react'
...
export default function RASListPage() {
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id)
    try {
      const res = await fetch(`/api/ras/familias/${id}`, { method: 'DELETE' })
      if (res.ok) {
                    ...
                    return (
                      <tr key={f.id}
                        className={`border-b border-stone-100 transition-colors ${
                          isConfirming ? 'bg-red-50' : 'hover:bg-stone-50'
                        }`}>
