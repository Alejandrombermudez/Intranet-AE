...
export default function SiembraListPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error ?? 'Error al eliminar la familia.')
      }
    } finally {
      setDeletingId(null)
      setConfirmId(null)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Este select pide columnas que `migration_campo_core.sql` eliminó el
      // 2026-07-07 (nombre_propietario, municipio, vereda, nombre_finca se
      // leen ahora por JOIN a core.predios/core.aliados). Devuelve 42703 y
      // `data` en null: sin esto la pantalla decía "0 familias" como si la
      // base estuviera vacía, y no lo está. Mentir es peor que fallar.
  ...
  return (
    <div className="min-h-screen bg-papel">
      ...
      <Cabecera
        ...
        acciones={<>
          <Boton variante="claro" href="/intranet/siembra/eliminadas" icono={<Archive size={14} />}>Eliminadas</Boton>
          <Boton variante="luz" href="/intranet/siembra/nueva" icono={<Plus size={14} />}>Nueva familia</Boton>
        </>}
      />
      ...
      <main className="max-w-7xl mx-auto px-6 sm:px-10 py-10">
        ...
        {errorCarga && (
          <div className="mb-8 flex items-start gap-3 bg-red-50 border border-red-200 rounded-2xl px-5 py-4">
            ...
            <div className="text-sm text-red-800 space-y-1">
              <p className="font-black">Este listado no está leyendo la base</p>
              <p className="leading-relaxed">
                La consulta pide columnas que se eliminaron de <code className="font-mono text-xs">siembra.familias</code> en
                el rediseño del 2026-07-07: el propietario, el municipio, la vereda y el nombre de la finca ahora
                viven en <code className="font-mono text-xs">core.predios</code> y <code className="font-mono text-xs">core.aliados</code>.
                Hay familias registradas; no se están mostrando. Pendiente de arreglar — el módulo necesita resolver
                la identidad por JOIN a <code className="font-mono text-xs">core</code>, como ya se hizo en GeoAE.
              </p>
              <p className="text-xs opacity-70 font-mono">{errorCarga}</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-primary text-white rounded-2xl p-6 mb-8 shadow-xl">
          <div className="flex items-start gap-4">
            <Trees size={36} className="text-white/80 shrink-0 mt-1" />
            <div>
              ...
              <p className="text-white/70 text-sm">
                ...
                {familias.length === 1 ? 'familia vinculada' : 'familias vinculadas'} al proceso de restauración.
              </p>
            </div>
          </div>
        </div>
        ...
        {familias.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-stone-400 gap-4">
            <div className="w-20 h-20 rounded-3xl bg-stone-100 flex items-center justify-center">
              <Leaf size={40} className="text-stone-300" />
            </div>
            ...
            <Link href="/intranet/siembra/nueva"
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-stone-50 border-b border-stone-200">
                    {['Propietario / Finca', 'Municipio', 'Vereda', 'Ha. Restauración', 'Registrado', 'Acciones'].map((h, i) => (
                  ...
                  {familias.map((f) => {
                    ...
                    return (
                      <tr key={f.id}
                        ...
                        <td className="px-4 py-3 text-sm text-stone-500">{f.vereda ?? <span className="text-stone-300">—</span>}</td>
                        <td className="px-4 py-3">
                          {f.ha_restauracion != null ? (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-bold">
                              <Trees size={11} /> {f.ha_restauracion} ha
                            </span>
